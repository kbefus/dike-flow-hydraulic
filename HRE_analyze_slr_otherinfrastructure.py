...
Purpose: Script used to post-process dike flow model described in Section 4.3 and shown in Figure 8.
...
time_delta=1800
#%%
for n_flap,n_sluice in infra_combos:
    for mar_col in mar_cols:
        print('---------------')
        print(mar_col)
        scen_file = os.path.join(r'F:\herring\slr','hr_{0}_{1:1.0f}_dt{2:d}s_nsluice{3:d}_nflap{4:d}_220324.csv.gz'.format(mar_col,sval,time_delta,n_sluice,n_flap))
        rname = '_'.join(os.path.basename(scen_file).split('_')[3:5])
        print(rname)
        ...
        mean_df = temp_df.rolling(avg_window,axis=0,center=True,min_periods=int(avg_window/10)).mean() # Calculate rolling average
        mean_df = mean_df.resample(new_dt).asfreq() # Resample to annual timestep

        
        # Percent change in rate - how quickly is the estuary hydrology changing?
        mean_df['dVol_p'] = (100*(mean_df[vol_col] - mean_df[vol_col].iloc[0])/mean_df[vol_col].iloc[0]) # % relative volume change normalized to present
        mean_df['dArea_p'] = (100*(mean_df[a_col] - mean_df[a_col].iloc[0])/mean_df[a_col].iloc[0]) # % relative area change normalized to present
        ...
        if 'drained' not in flow_ratio.columns:
            flow_ratio['drained'] = 0. # never drained
        if 'impounded' not in flow_ratio.columns:
            flow_ratio['impounded'] = 0. # never impounded
            
        flow_ratio_list.update({'{}_{}_s{}f{}'.format(rname,'drained',n_sluice,n_flap):flow_ratio['drained'],
                                '{}_{}_s{}f{}'.format(rname,'impounded',n_sluice,n_flap):flow_ratio['impounded']})
        

out_flow_ratios = os.path.join(annual_dir,'all_flow_ratios_otherinfra_220325.csv')
fdr = pd.DataFrame(flow_ratio_list)
fdr.to_csv(out_flow_ratios,index=False)
                                
#%%
# This can be moved to a new script for plotting only, and was. See: HR_plot_otherinfrastructure_drainability_Figure8.py
plot=True
if plot:
    ...
    for islr,slr_scen in enumerate(slr_scens):
        ...
        for ih, hydro_val in enumerate(hydro_vals):
            ...
            save2dhist = os.path.join(annual_dir,'{}_{}_ratio_2dhist_otherinfra_220325.csv'.format(hydro_val,slr_scen))
