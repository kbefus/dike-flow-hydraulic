...
Script used to run dike flow model described in Section 4.2
...
high_e_df = s_df.loc[s_df['rnp']>.90] # rnp>90 = 249, 
rerun=False
...
#%% Define hydraulic parameters for the control structures
    for ihigh in np.arange(high_e_df.shape[0])[::-1]:
        ...
        out_fname = os.path.join(r'F:\herring\slr','hr_{0}_{1:1.0f}_dt{2:d}s_220309.csv.gz'.format(mar_col,val_df.name,time_delta))
